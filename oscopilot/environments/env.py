...
from oscopilot.environments import PowerShell
...
class Env(BaseEnv):
    ...
    def __init__(self):
        ...
        self.languages = [Shell]
        
        # Try to use PythonJupyterEnv, fall back to PythonSimpleEnv if not available
        try:
            from oscopilot.environments import PythonJupyterEnv
            self.languages.insert(0, PythonJupyterEnv)
        except ImportError:
            from oscopilot.environments import PythonSimpleEnv
            self.languages.insert(0, PythonSimpleEnv)
        
        # Add platform-specific languages
        system = platform.system()
        if system == "Darwin":
            # Add macOS-specific languages
            self.languages.append(AppleScript)
        elif system == "Windows":
            # Add Windows-specific languages
            from oscopilot.environments import PowerShell
            self.languages.append(PowerShell)
